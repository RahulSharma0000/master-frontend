...
export default function EditLanguage() {
  ...
  useEffect(() => {
    ...
    const fetched = { name: "English", code: "EN", default: true };
    setForm(fetched);
  }, [id]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Update Language:", form);
  ...
  return (
    <MainLayout>
      ...
      >
        <Input label="Language Name" name="name" value={form.name} onChange={handleChange} required />
        <Input label="Language Code" name="code" value={form.code} onChange={handleChange} required />

        <div className="flex items-center mt-4 md:col-span-2 gap-3">
          <input type="checkbox" name="default" checked={form.default} onChange={handleChange} className="w-5 h-5" />
        ...
        <div className="md:col-span-2 flex justify-end mt-4 gap-2">
          <button type="button" onClick={() => navigate("/controls/language")} className="px-5 py-3 rounded-xl border text-gray-700">Cancel</button>
          <button type="submit" className="px-5 py-3 rounded-xl text-white bg-blue-600 flex items-center gap-2 hover:bg-blue-700">
            <FiSave /> Update Language
          </button>
        </div>
      </form>
    </MainLayout>
